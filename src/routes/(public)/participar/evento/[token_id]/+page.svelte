<script lang="ts">
	...
	import { Calendar, CheckCircle, Loader2, Ticket, XCircle } from 'lucide-svelte';
	...
	const claimMutation = createMutation(() => ({
		...
		onSuccess: (evt) => {
			...
			goto(`/eventos/${token?.organization_slug}/${evt.slug ?? token?.event_slug}`);
		},
		onError: () => {
	...
	function handleClaim() {
		if (!isAuthenticated) {
			goto(`/login?redirect=/participar/evento/${tokenId}`);
			return;
		}
	...
	const formattedDate = $derived(token ? formatEventDate(token.event_start) : '');
...
<svelte:head>
	{#if rejection}
		<title>{m['joinEventPage.rejectedTitle']()} - Revel</title>
	{:else if token}
		...
		<Card class="w-full max-w-md">
			...
			<CardContent class="space-y-6">
				<p class="text-center text-sm text-muted-foreground">
					{m['joinEventPage.rejectedHint']()}
				</p>
				<Button size="lg" class="w-full" onclick={() => goto('/eventos')}>
