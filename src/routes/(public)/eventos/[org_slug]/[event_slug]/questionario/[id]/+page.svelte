<script lang="ts">
	...
	const eventUrl = $derived(`/eventos/${data.event.organization.slug}/${data.event.slug}`);
...
<div class="container mx-auto max-w-3xl px-4 py-8">
	...
	<div class="mb-8">
		<a
			href="/eventos/{data.event.organization.slug}/{data.event.slug}"
		...
		<form onsubmit={handleSubmit} class="space-y-8">
			...
			<div class="flex items-center justify-end gap-4 border-t pt-6">
				<Button
					type="button"
					variant="outline"
					onclick={() => goto(`/eventos/${data.event.organization.slug}/${data.event.slug}`)}
				>
